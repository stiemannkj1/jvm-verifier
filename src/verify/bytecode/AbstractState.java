...
/**
 * Class AbstractState is the verification-time version of the runtime state. It is final mostly for
 * performance reasons.
 *
 * <p>Let's face it, .. we have many many instances of AbstractState. However, I tried once with a
 * very clever "copy-on-write" design, but it turned out that it was not worth the overhead. So,
 * why? Well, I guess it's because garbage is so cheap. If you allocate a lot of memory, but don't
 * use it, then it's almost free.
 */
public final class AbstractState implements verify.type.TypeTags {
  ...
  */
  ...
  void push_for_jsr(int return_pc) {
    for (AbstractCallStack next = jsrStack; next != null; next = next.next) {
      if (return_pc == next.return_pc) return;
    }

    jsrStack = new AbstractCallStack(return_pc, jsrStack);
  }

  void pop_for_ret(int return_pc) throws verify.VerificationException {
    ...
    if (i > 1) System.out.println("@@@@ interesting, multi level jsr @@@@");
  }
  ...
  */
  ...
  /**
   * make a new abstract state, as a copy of <code>state</code>. We set the pc to -1, to notify that
   * this state is "free". When the pc is set, that means that the state is "frozen", and must stay
   * on the state list for that pc.
   */
  public AbstractState(AbstractState state) {
    ...
    System.arraycopy(state.storage, 0, storage, 0, state.storage.length);
  }

  /**
   * Return the state, that an exception handler would see, if <code>exception</code> was thrown
   * here.
   */
  public AbstractState(AbstractState state, Type exception) throws verify.VerificationException {
    pc = -1;
    jsrStack = state.jsrStack;
    constructor_start = state.constructor_start;
    context = state.context;
    ...
    storage = new Type[state.storage.length];
    System.arraycopy(
        state.storage, max_stack, storage, max_stack, state.storage.length - max_stack);
  ...
  boolean exceptionMayMerge(AbstractState state) {
    ...
    for (int i = max_stack(); i < max; i++) {
      ...
      if (this_type == other_type) continue;

      if (this_type == null) continue;

      if (other_type == null) return true;

      if (this_type.storageClass == SC_INT && other_type.storageClass == SC_INT) continue;
   ...
   */
  public boolean merge(TypeContext ctx, AbstractState other) throws verify.VerificationException {
  ...
  /** "equality" */
  public boolean equals(AbstractState other) throws verify.VerificationException {
    if (other.sp != sp) {
  ...
  public void push(Type type) throws verify.VerificationException {
    if (type == null) throw new InternalError("pushing null value!");
  ...
  public void push(Type type1, Type type2) throws verify.VerificationException {
    if (sp + 1 >= max_stack()) {
      throw new verify.VerificationException("stack overflow: push(" + type1 + ", " + type2 + ")");
    }
    Type[] wstate = storage;
  ...
  /* unchecked pop */
  public Type pop() throws verify.VerificationException {
    if (sp < 1) {
  ...
  public Type pop(int storage_class) throws verify.VerificationException {
    if (sp < 1) {
      throw new verify.VerificationException("stack underflow: pop()");
    }
    ...
    if (type.storageClass != storage_class) {
      throw new IncompatibleTypesException("unexpected value on stack: pop()", type, storage_class);
    }

    return type;
  }

  public void pop(int sc0, int sc1) throws verify.VerificationException {
    if (sp < 2) {
      throw new verify.VerificationException("stack underflow: pop()");
    }
    ...
    if (type1.storageClass != sc1 || type0.storageClass != sc0) {
      throw new IncompatibleTypesException("unexpected value on stack", type0, sc0);
    }
  }

  /* unchecked pop of n stack elements */
  public void popn(int n) throws verify.VerificationException {
    if (sp < n || n < 0) {
  ...
  public Type peek(int n, int storage_class) throws verify.VerificationException {
    if (n < 1 || n > sp) {
  ...
  public Type read(int n, int storage_class) throws verify.VerificationException {
    if (n < 0 || n > max_locals()) {
    ...
    if (type == null)
      throw new verify.VerificationException("reading uninitialized variable, local[" + n + "]");
  ...
  public void write(int n, Type type) throws verify.VerificationException {
    if (n < 0 || n > max_locals()) {
      throw new verify.VerificationException("out of bounds: write(" + n + ", " + type + ")");
    }
    (storage)[max_stack() + n] = type;
  }

  /** .., class1, class2 -> .. */
  public void store(int n, int class1, int class2) throws verify.VerificationException {
    ...
    if (type1.storageClass != class1) {
      throw new IncompatibleTypesException("bad value on stack: store/2(" + n + ")", type1, class1);
    }
    wstate[index] = type1;
  }

  /** .., class1 -> .. */
  public void store(int n, int class1) throws verify.VerificationException {
    int max_locals = max_locals();
    int max_stack = max_stack();
  ...
  /** ..., -> ..., class1, class2 */
  public void load(int n, int class1, int class2) throws verify.VerificationException {
    int max_locals = max_locals();
    int max_stack = max_stack();
  ...
  /** ..., -> ..., class1 */
  public void load(int n, int class1) throws verify.VerificationException {
  ...
  void aload(Type elemType, Type extra) throws verify.VerificationException {
    pop(SC_INT);
    ...
    if (!array.isAnonymousArray()) {
      if (array.tag != T_ARRAY)
        throw new verify.VerificationException("non-array operand to aaload insn");
    ...
    if (extra != null) push(elem, extra);
    else push(elem);
  }

  void astore(Type elemType, int sc2) throws verify.VerificationException {
    if (sc2 != 0) pop(sc2);
    ...
    if (!array.isAnonymousArray()) {
      if (array.tag != T_ARRAY)
        throw new verify.VerificationException("non-array operand to array store insn");

      elem = ((ArrayType) array).elementType;
    }

    if (elemType.storageClass == SC_ADDR) elem.checkAssignmentFrom(value);
    else if (elem != elemType)
      throw new verify.VerificationException(
          "" + array + " operand to " + elemType + "[] store operation");
  }

  public void dupx(int n, int x) throws verify.VerificationException {
    if (sp + n > max_stack()) {
  ...
  void swap() throws verify.VerificationException {
    Type[] wstate = storage;
    if (sp < 2) throw new verify.VerificationException("stack underflow in swap() operation");
    Type tmp = wstate[sp - 1];
    wstate[sp - 1] = wstate[sp - 2];
    wstate[sp - 2] = tmp;
  }

  void unop(Type type0) throws verify.VerificationException {
    if (sp < 1) throw new verify.VerificationException("stack underflow, missing argument!");
  ...
  void unop(Type type0, Type type1) throws verify.VerificationException {
    if (sp < 2) throw new verify.VerificationException("stack underflow, missing argument!");
  ...
  void binop(Type type0) throws verify.VerificationException {
    Type[] wstate = storage;

    if (sp < 2) throw new verify.VerificationException("stack underflow in " + type0 + " binop");
  ...
  void binop(Type type0, Type type1) throws verify.VerificationException {
    Type[] wstate = storage;

    if (sp < 4) throw new verify.VerificationException("stack underflow in " + type0 + " binop");
  ...
  public String dump(boolean stack_only) {
    StringBuffer b = new StringBuffer("*** state of " + context.name() + " *** \n");
    ...
    for (int i = 0; i < rstate.length; i++) {
      if (i >= sp && i < max_stack()) {
        ...
        if (!stack_only) b.append("local[" + (i - max_stack()) + "] = ?\n");
      } else {
        if (i < max_stack()) b.append("stack[" + i + "] = " + rstate[i] + "\n");
        else if (!stack_only) b.append("local[" + (i - max_stack()) + "] = " + rstate[i] + "\n");
      }
    }
  ...
  ClassType replaceConstructedObject(Type object) {
    ...
    for (int i = 0; i < wstate.length; i++) {
      if (wstate[i] == child) wstate[i] = child.klass;
    }
    return child.klass;
  }
...
class AbstractCallStack {
  ...
  public String toString() {
    if (next == null) return Integer.toString(return_pc);
    else return next.toString() + " " + return_pc;
  }
}
