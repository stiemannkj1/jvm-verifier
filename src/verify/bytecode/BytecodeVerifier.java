...
public class BytecodeVerifier implements InsnNames, TypeTags, ClassFileConstants {
  ...
  /**
   * This array registers the "mode" of each byte in the byte code sequence. All bytes start off as
   * "unknown", and once assigned a value, it *must* not change. At the end of verification, all
   * bytes that do not have an assigned mode constitutes unreachable code. The JVM Specification
   * does not state that unreachable code is illegal, so we just emit a warning.
   */
  ...
  AbstractState get_next_state(int[] pc) throws verify.VerificationException {
    ...
    if (state != null) {
      ...
      if (pc[0] == -1) throw new InternalError("next state has pc -1");

      state = state.copy();

      if (state.pc() != -1) throw new InternalError("copy yields pc != -1");
    }
    return state;
  }

  private AbstractState findApplicableState(AbstractState state, int target_pc) {
    int logic_pc = target_pc - pc_offset;
    for (AbstractState s = pc_states[logic_pc]; s != null; s = s.next_this_pc) {
      if (s.pc() != target_pc) throw new InternalError("pc mismatch in list for pc=" + logic_pc);

      if (state.hasSameJsrStack(s)) return s;
    }
    return null;
  }

  void branch(AbstractState state, int target_pc) throws verify.VerificationException {
    if (target_pc >= end_pc || target_pc < pc_offset)
      throw new verify.VerificationException("control flow reaches outside of the code");
  ...
  void conditional_branch(AbstractState state, int target_pc1, int target_pc2)
  ...
  void check(ClassFile.Method method) {
    System.err.print(".");
    ...
    try {
      ...
      System.out.println(classfile.getName() + " is not an allowed class name");
      return;
    }
    ...
    if (code == null) {
      System.err.println("non-native, non-abstract method " + mname + " has no code attribute.");
      return;
    }
    ...
    try { // this is the entire thing...
      ...
      if (pc_states == null || pc_states.length != code_size || byte_mode == null)
      ...
      /**
       * Establish initial state. First, store the receiver, "this", if the method is non-static. If
       * so, this will offset the rest of the arguments by one.
       */
      int arg_offset = 0;
      ...
      while (true) {
        ...
        while (true) {
          ...
          if ((++insn_count % 1000) == 0) System.err.print(":");

          /* This checks that we are not executing code out-of-bounds */

          if (pc >= end_pc || pc < pc_offset)
            throw new verify.VerificationException("pc=" + (pc - pc_offset) + " is out of bounds");
          ...
          switch (opcode) {
            ...
            case op_aaload: /* 0x32 */
              {
                ...
                if (array.tag != T_ARRAY && array.tag != T_NULL)
            ...
            case op_bastore: /* 0x54 */
              {
                ...
                if (!array.isAnonymousArray()) {
                  ...
                  if (type != ctx.BYTE && type != ctx.BOOLEAN)
                    throw new verify.VerificationException(
                        "" + array + " operand to " + "(byte|boolean)" + "[] store operation");
                }
              }
              continue next_insn;
            ...
            case op_tableswitch: /* 0xaa */
              {
                ...
                while (((pc - pc_offset) % 4) != 0) {
                  if (code[pc] != 0)
                    warn("padding byte '0x" + Integer.toHexString(code[pc - 1]) + "' is non-zero");
                  pc++;
                }
            ...
            case op_ireturn: /* 0xac */
              ...
              if (sig.returnType().storageClass != SC_INT)
                throw new verify.VerificationException("erroneous return type");
              break next_insn;
            ...
            case op_putfield: /* 0xb5 */
              {
                ...
                if (state.constructor_start) {
                  ...
                  if (self != thisType) {
                    throw new verify.VerificationException(
                        "illegal field assignment in " + "constructor code");
                  }
                } else {
                  Type object = state.pop(SC_ADDR);
                  fclass.checkAssignmentFrom(object);
                  ...
                  if (receiver.tag == T_NEW) {
                    if (is_constructor && opcode == op_invokespecial) {
                      ...
                      if (state.constructor_start && receiver == thisType) {
                        ...
                        if (expected_receiver != thisType && expected_receiver != superClass)
                          throw new verify.VerificationException("illegal constructor call");
                      ...
                      throw new verify.VerificationException(
                          "Method " + msigname + " called with an " + "uninitialized object.");
                    }
                  } else if (is_constructor) {
                    throw new verify.VerificationException(
                        "Constructor" + " called with an " + "initialized object.");
                  }
      ...
      */
      ...
      System.err.println("non-verificatrion exception, " + ex.getMessage());
      ex.printStackTrace();
      System.exit(1);
          ...
          case BYTE_MODE_UNKNOWN:
            {
              ...
              if (!found_unreachable_code) {
                System.err.print(" unreachable in " + method.name() + ": ");
                found_unreachable_code = true;
              } else {
                System.err.print(", ");
              }

              System.err.print("" + start + "+" + (i - 1 - start) + "");
            }
  ...
  Type getClassType(int index) throws verify.type.IllegalTypeNameException {
    return ctx.classFor(classfile.pool.get_class(index));
  }

  Type getFieldType(int index) throws verify.type.IllegalTypeNameException {
    return ctx.typeForName(classfile.pool.get_fieldref_type(index));
  }

  Type getFieldClass(int index) throws verify.type.IllegalTypeNameException {
    return ctx.classForName(classfile.pool.get_fieldref_class(index));
  }

  Signature getMethodSignature(int index) throws verify.type.IllegalTypeNameException {
    String sig = classfile.pool.get_methodref_type(index);
    return Signature.forString(ctx, sig);
  }

  Signature getInterfaceMethodSignature(int index) throws verify.type.IllegalTypeNameException {
    String sig = classfile.pool.get_interfacemethodref_type(index);
    return Signature.forString(ctx, sig);
  }

  String getInterfaceMethodName(int index) throws verify.type.IllegalTypeNameException {
    return classfile.pool.get_interfacemethodref_name(index);
  }

  String getMethodName(int index) throws verify.type.IllegalTypeNameException {
    return classfile.pool.get_methodref_name(index);
  }

  String getInterfaceMethodClass(int index) throws verify.type.IllegalTypeNameException {
    return classfile.pool.get_interfacemethodref_class(index);
  }

  String getMethodClass(int index) throws verify.type.IllegalTypeNameException {
    return classfile.pool.get_methodref_class(index);
  }

  Type getConstantType(int index) throws verify.VerificationException {
    switch (classfile.pool.get_tag(index)) {
      ...
      default:
        throw new verify.VerificationException(
            "did not find a constant value at index " + index + " in constant pool");
    }
  }

  Type find_receiver(Signature sig, AbstractState state, boolean new_object)
      throws verify.VerificationException {
    int nargs = sig.numberArgs();
    int offset = 0;
    for (int arg = nargs - 1; arg >= 0; arg--) {
      Type targ = sig.declaredArgument(arg);
      if (targ.tag == T_LONG || targ.tag == T_DOUBLE) offset += 2;
      else offset += 1;

      /* log ("peeking " + state.peek (offset, targ.storageClass)); */
    }

    if (new_object) return state.peek(offset + 1, T_NEW);
    else return state.peek(offset + 1, SC_ADDR);
  }

  Type pop_arg(Type argtype, AbstractState state) throws verify.VerificationException {
