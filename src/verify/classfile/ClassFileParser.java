...
/**
 * A <code>ClassFileParser</code> is used for parsing class files (for whatever reason). It has a
 * single entrypoint, the static method <code>parse</code>. This method is given the class file
 * information, as an array of bytes, and an instance of a subclass of <code>ClassFileHandler</code>
 * . As the parser progresses through the file, it will call
 */
public class ClassFileParser implements ClassFileConstants {
  ...
  protected void handleField(int findex, int access_flags, int name_index, int descriptor_index) {}

  protected void handleConstantValueAttribute(int field_index, int valindex) {}

  protected void handleFieldsEnd() {}

  protected void handleMethodsBegin(int count) {}

  protected void handleMethod(int mth_index, int acc, int name, int desc) {}
  ...
  protected void handleExceptionTableEntry(
      int method_index, int exc_index, int s, int e, int h, int c) {}

  protected void handleExceptionsAttribute(int method_id, int[] exceptions) {}

  protected void handleMethodsEnd() {}

  protected void handleSourceFileAttribute(int idx) {}

  protected void handleUnknownAttribute(int name, byte[] data, int att_off, int att_len) {}
  ...
  private final boolean equals_utf8(int name, String s) {
    ...
    for (int i = 0; i < len; i++) {
      if (bytes[i + where + 2] != s.charAt(i)) return false;
    }
    return true;
  }
  ...
  protected ClassFileParser(byte[] i, int off, int len) {
  ...
  private final void check(int num) {
    if (offset + num > start + length) throw new ClassFormatError("Premature end of data");
  }
  ...
  private int read_one_method_attribute(int method_index) {
    ...
    if (equals_utf8(name, "Exceptions")) {
      ...
      handleCodeAttributeBegin(
          method_index, max_stack, max_locals, bytes, code, code_length, exception_table_length);
      ...
      for (int i = 0; i < exception_table_length; i++) {
        ...
        if (start_pc > end_pc
            ...
            || end_pc > code_length
            || handler_pc > code_length) throw new ClassFormatError();
  ...
  private void read_methods() {
    ...
    for (int i = 0; i < methods_count; i++) {
      ...
      handleMethod(i, access_flags, name_index, descriptor_index);
