...
public class n2h {
  ...
  public static int get2s(byte[] data, int off) {
    return (((int) data[off]) << 8) | (((int) data[off + 1]) & 0x00ff);
  }

  public static int get4(byte[] data, int off) {
    return ((((int) data[off]) << 24) & 0xff000000)
  ...
  public static long get4u(byte[] data, int off) {
    return ((((long) data[off]) << 24) & 0xff000000)
  ...
  public static long get8(byte[] data, int off) {
    return ((long) get4(data, off)) << 32 | (((long) get4(data, off + 4)) & 0xffffffffL);
  }
  ...
  /**
   * Decode a Java-style UTF byte sequence. Doing it ourselves seems to be *much* faster than
   * calling
   *
   * <p>new String (data, off, len, "UTF8");
   *
   * <p>Which does a whole lot of meta-decoding stuff, and it might very well be implemented in Java
   * anyway.
   */
