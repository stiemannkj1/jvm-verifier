...
public final class ConstantPool implements ClassFileConstants {
  ...
  final void check_tag(int index, int tag) {
    if (index > size || index < 0) throw new ClassFormatError("constant pool index out of range");
  ...
  public final int get_tag(int index) {
    if (index < 1 || index >= size) throw new IllegalArgumentException();
  ...
  /**
   * get_index(0|1) comes in two variations, a checked an a non-checked. The non-checked is supposed
   * to be used when doing subsequent reads, in a context, where it is nown to be sound.
   */
  private final IndexPair load_two_indexes(int index) {
    IndexPair pair =
        new IndexPair(n2h.get2u(bytes, offsets[index]), n2h.get2u(bytes, offsets[index] + 2));
    cache[index] = pair;
    return pair;
  }
  ...
  private final int get_index0(int index) {
    ...
    if (pair == null) return load_two_indexes(index).index0;
    else return pair.index0;
  }
  ...
  private final int get_index1(int index) {
    IndexPair pair = (IndexPair) cache[index];
    if (pair == null) return load_two_indexes(index).index1;
    else return pair.index1;
  }
