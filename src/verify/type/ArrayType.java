...
public class ArrayType extends Type {
  ...
  static ArrayType forName(TypeContext ctx, String name, int start, int[] end)
      throws IllegalTypeNameException {
    if (name.charAt(start) == '[') {
      int array = 1;
      while (name.charAt(start + array) == '[') array++;
      Type type = ctx.typeForName(name, start + array, end);
      while (array-- > 1) type = ArrayType.forElement(type);
  ...
  public static ArrayType forElement(Type element) {
    if (element == null) throw new IllegalArgumentException("ArrayType.forElement(null)");

    if (element.arrayType == null) element.arrayType = new ArrayType(element);
  ...
  protected void subclassCheckAssignmentFrom(Type type) throws verify.VerificationException {
  ...
  public Type mergeWith(Type other) throws verify.VerificationException {
    ...
    if (other.tag == T_ARRAY) {
      ...
      if (merged == null) return context.OBJECT;
    ...
    if (other.tag == T_CLASS) return context.OBJECT;
