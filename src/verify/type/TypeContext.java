...
/**
 * Class TypeContext is the nursery for type objects. "Global" variables are kept in this object, so
 * that we are sure that we can garbage collect the entire thing away after we're done verifying.
 */
public class TypeContext implements TypeTags {
  ...
  /** The next two types, represent "the second half" of a long or double in the abstract state */
  ...
  public Type classFor(String name) throws IllegalTypeNameException {
    if (name.charAt(0) == '[') return ArrayType.forElement(typeForName(name.substring(1)));
    else return classForName(name);
  }

  private final java.util.Hashtable classes = new java.util.Hashtable();

  public ClassType classForName(String className) throws IllegalTypeNameException {
  ...
  public Type typeForName(String name) throws IllegalTypeNameException {
    int[] end = new int[1];
    Type result = typeForName(name, 0, end);

    if (end[0] != name.length()) throw new IllegalTypeNameException(name);
    return result;
  }

  public Type typeForName(String name, int start, int[] end) throws IllegalTypeNameException {
    switch (name.charAt(start)) {
      ...
      case 'V':
        ...
        return VOID;
      case '[':
        return ArrayType.forName(this, name, start, end);
      case 'L':
        ...
        throw new IllegalTypeNameException("no end marker ';' in " + name.substring(start));
