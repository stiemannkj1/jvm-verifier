...
public class ClassType extends Type {
  ...
  public void loadFrom(ClassInfo info) throws IllegalTypeNameException {
    if (this == context.OBJECT) return;

    if (info == null) {
      /* do something meaningful, assume that we're a subclass
      of object, if nothing else */

      if (super_class == null) super_class = context.OBJECT;
      return;
    }
  ...
  private void load() throws verify.VerificationException {
    ...
    if (!i_am_loaded) {
      ...
      try {
        ...
        if (info == null) throw new verify.VerificationException("cannot load class " + this);
        loadFrom(info);
    ...
    if (super_class != null) super_class.load();
  ...
  String shortName() {
    ...
    if (dot == -1) return class_name;
    else return "..." + class_name.substring(dot + 1);
  }

  public boolean isSubtypeOf(ClassType otherClass) throws verify.VerificationException {
    if (this == otherClass) {
      return true;
    }

    if (super_class == null) load();
  ...
  public Type mergeWith(Type other) throws verify.VerificationException {
    if (other == null) return null;

    if (this == other) return this;

    if (other.tag == T_CLASS) return commonSuperWith((ClassType) other);

    if (other.storageClass != SC_ADDR) return null;

    if (other.tag == T_ARRAY) return context.OBJECT;

    if (other.tag == T_NULL) return this;
    else return null;
  }

  public void subclassCheckAssignmentFrom(Type other) throws verify.VerificationException {
