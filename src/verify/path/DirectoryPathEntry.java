...
final class DirectoryPathEntry extends PathEntry {
  ...
  DirectoryPathEntry(File f) throws java.io.IOException {
    if (!f.isAbsolute()) throw new IllegalArgumentException();
  ...
  URL getURL(String file) {
    ...
    try {
      if (!f.getCanonicalPath().startsWith(base_canon)) throw new IllegalArgumentException(file);

      if (in_cache(f)) return new URL("file", "", f.getPath());
      else return null;
  ...
  InputStream getStream(String file) {
    ...
    try {
      ...
      if (in_cache(f)) return new FileInputStream(f);
      else return null;
    } catch (IOException x) {
      return null;
    }
  ...
  byte[] getBytes(String file) {
    ...
    try {
      ...
      if (in_cache(f)) return readbytes(new FileInputStream(f), (int) f.length());
      else return null;
    } catch (IOException x) {
      return null;
    }
