...
final class ZipPathEntry extends PathEntry {
  ...
  ZipPathEntry(File f) throws MalformedURLException, ZipException, IOException {
  ...
  */
  ...
  URL getURL(String f) {
    ...
    try {
      if (ent != null) return new URL(file, "#" + f);
      else return null;
    } catch (IOException x) {
      return null;
    }
  ...
  InputStream getStream(String f) {
    ...
    try {
      if (ent != null) return zip.getInputStream(ent);
      else return null;
    } catch (IOException x) {
      return null;
    }
  ...
  byte[] getBytes(String f) {
    ...
    try {
      if (ent != null) {
        return readbytes(zip.getInputStream(ent), (int) ent.getSize());
      } else {
        return null;
      }
    } catch (IOException x) {
